...
const { BadRequestError, UnauthorizedError, NotFoundError, InternalServerError, ForbiddenError } = require('../lib/errors');
...
router.post('/removeWorkspace', async (request, response, next) => {
    ...
    if (!wid) {
        return next(new BadRequestError(`There is no wid. request.body: ${util.inspect(request.body, true, 2, true)}`));
    }
    if (sessionManager.isLiveSession(parseInt(wid))) {
        return next(new InternalServerError(`This Workspace(${wid}) is running in other session`));
    }
    ...
    try {
        ...
        if (res1.length !== 1 || res1[0].owner !== uid) {
            throw new ForbiddenError(`The user has no ownership or the wid is invalid. wid: ${wid}`);
        }
...
router.post('/setBookmark', async (request, response, next) => {
    ...
    if (isNaN(wid) || isNaN(val) || val > 1 || val < 0) {
        return next(new BadRequestError(`Invalid wid or val. request.body: ${util.inspect(request.body, true, 2, true)}`));
    }
    ...
    try {
        ...
        if (res1.affectedRows === 0) {
            throw new ForbiddenError(`The user did not participate the workspace. val: ${val}, uid: ${uid}, wid: ${wid}`);
        }
...
router.get('/wsList', async (request, response, next) => {
    ...
    if (isNaN(page) || isNaN(rows) || isNaN(order) || order > 3 || desc === undefined || bmonly === undefined) {
        return next(new BadRequestError(`Invalid parameters. request.query: ${util.inspect(request.query, true, 2, true)}`));
    }
...
router.post('/invite', async (request, response, next) => {
    ...
    if (!receiver || !workspaceId) {
        return next(new BadRequestError(`Invalid invitation receiver or workspace id. request.body: ${util.inspect(request.body, true, 2, true)}`));
    }
    ...
    try {
        ...
        if (res1[0].cnt !== 1) {
            throw new ForbiddenError(`The user has no permission to invite sommeone. senderId: ${senderId}, wid: ${workspaceId}`);
        }

        const res2 = await conn.query(`select id from t_user where ${field} = ?`, receiver);
        if (res2.length !== 1) {
            throw new BadRequestError(`The invitation receiver doese not exist. receiver: ${receiver}`);
        }
        ...
        if (res3[0].cnt !== 0) {
            throw new BadRequestError(`The invitation receiver has already participated. receiverId: ${receiverId}, wid: ${workspaceId}`);
        }
...
router.post('/join', async (request, response, next) => {
    ...
    if (!iid) {
        return next(new BadRequestError(`Invalid invitation id. request.body: ${util.inspect(request.body, true, 2, true)}`));
    }
    ...
    try {
        ...
        if (res1.length !== 1) {
            throw new BadRequestError(`The user has not been invited. uid: ${uid}, invitation id: ${iid}`);
        }
...
router.get('/manage', async (request, response, next) => {
    ...
    if (!wid) {
        return next(new BadRequestError(`Invalid wid. request.query: ${util.inspect(request.query, true, 2, true)}`));
    }
    ...
    try {
        ...
        if (res1.length === 0) {
            throw new ForbiddenError(`The user has no role in that workspace. uid: ${uid}, wid: ${wid}`);
        }
        ...
        if (hasAuth) {
            ...
            if (res2.length === 0) {
                throw new ForbiddenError(`Nonexistent workspace access. wid: ${wid}`);
            }
            ...
            throw new ForbiddenError(`The user does not have authority to manage workspace. authority: ${authority}`);
        }
...
router.post('/alter', async (request, response, next) => { //todo: Check the content source validation
    ...
    if (!wid || !wname) {
        return next(new BadRequestError(`Invalid wid or wname. request.body: ${util.inspect(request.body, true, 2, true)}`));
    }
...
router.post('/leave', async (request, response, next) => {
    ...
    const wid = request.body.wid;
    if (!wid) {
        return next(new BadRequestError(`Invalid wid. request.body: ${util.inspect(request.body, true, 2, true)}`));
    }
...
router.post('/editParticipant', async (request, response, next) => {
    ...
    if (isNaN(wid) || isNaN(participant) || (type === '1' && isNaN(role)) || (type !== '1' && type !== '2')) {
        return next(new BadRequestError(`Invalid wid or participant or role or type. request.body: ${util.inspect(request.body, true, 2, true)}`));
    }
    ...
    try {
        ...
        if (res1[0].owner === participant) {
            throw new ForbiddenError(`The owner cannot change their role themselves.`);
        }
        ...
        if (res3.length !== 1) {
            throw new ForbiddenError(`The user is not the participant. participant: ${participant}, wid: ${wid}`);
        }
...
router.post('/save', async (request, response, next) => {
    ...
    if (!wid || !content || !screenshot) {
        return next(new BadRequestError(`Invalid wid or content or screenshot. request.body: ${util.inspect(request.body, true, 2, true)}`));
    }
